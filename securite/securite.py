...
def init_security(app):
    ...
    Priorité : si la clé interne (Gateway) est valide, on laisse passer sans aucun filtrage.
    """
    ...
    def filter_requests():
        ...
        # ---------------------------------------------------------
        # VÉRIFICATION PRIORITAIRE DE LA CLÉ INTERNE (Gateway)
        # ---------------------------------------------------------
        internal_key = request.headers.get("X-Internal-Key")
        expected_key = os.getenv("TAKO_API_KEY")

        # Si la clé interne correspond, c'est une requête légitime du Gateway
        # → Aucune restriction, on laisse passer immédiatement
        if internal_key and internal_key == expected_key:
            return None

        # ---------------------------------------------------------
        # Sinon, c'est une requête externe → Appliquer les protections
        # ---------------------------------------------------------
        ...
        # Aucun problème détecté, on laisse passer
        return None
